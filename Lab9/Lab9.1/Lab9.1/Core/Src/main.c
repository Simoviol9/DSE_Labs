...
int main(void)
{
  ...
  LL_ADC_WriteReg(ADC1, CR2, LL_ADC_ReadReg(ADC1,CR2) | 0x00000003);
  ...
  {
    /* USER CODE END WHILE */
	  finish_conversion = LL_ADC_ReadReg(ADC1,SR) & 0x0002;
	  	  if (finish_conversion != 0){
	  		  sample = LL_ADC_ReadReg(ADC1,DR);
	  		  half_period = offset_ADC + sample*gain_ADC;
	  	  }
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
