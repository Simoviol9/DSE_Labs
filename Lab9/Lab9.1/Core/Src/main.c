...
int main(void)
{
  ...
  MX_TIM3_Init();
  /* USER CODE BEGIN 2 */
  int EOC; // End Of Conversion flag
  ...
  {
    /* USER CODE END WHILE */
	  EOC = (LL_ADC_ReadReg(ADC1,SR) & LL_ADC_FLAG_EOC)); // End Of Conversion
		if (EOC) { // Check EOC
			voltage = LL_ADC_ReadReg(ADC1, DR) & 0x00FF;	// Get converted value
			//printf("EOC: %d | Voltage (raw 8-bit): %d\r\n", EOC ? 1 : 0, voltage);
			// timeInterval = MIN_FREQ_INC + ((MAX_FREQ_INC - MIN_FREQ_INC) * voltage) / 255;	// Compute interval
			float realVoltage = (3.3f * voltage) / 255.0f;
			//printf("Voltage: %.2f V\r\n", realVoltage);
			LL_ADC_ClearFlag_EOCS(ADC1);	// Clear EOC
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
}
